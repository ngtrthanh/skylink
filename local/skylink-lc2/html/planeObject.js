...
PlaneObject.prototype.isFiltered = function () {
    // Always show selected planes
    if (this.selected)
        return false;

    // Handle GHOSTS filter from URL parameter
    const urlParams = new URLSearchParams(window.location.search);
    const filterType = urlParams.get('filtertype');
    if (filterType === 'GHOSTS') {
        return !!(this.icaoType && this.icaoType.trim() !== '');
    }
    ...
    for (const filter of filters_active) {
        ...
        // If field doesn't exist or is empty, filter it out
        if (!fieldValue) {
            return true;
        }

        const upperFieldValue = fieldValue.toString().toUpperCase();

        // Handle exact match: starts with '='
        if (typeof pattern === 'string' && pattern.startsWith('=')) {
            const exactMatch = pattern.substring(1).toUpperCase();
            if (upperFieldValue !== exactMatch) {
                return true; // Filter out if not exact match
            }
        } else {
            // Handle partial match
            const upperPattern = pattern.toString().toUpperCase();
            if (upperFieldValue.indexOf(upperPattern) === -1) {
                return true; // Filter out if no partial match
            }
        }
    }
    ...
    // Data Source Filter
    if (PlaneFilter.sources?.length > 0 && !PlaneFilter.sources.includes(this.dataSource)) {
        return true;
    }
    ...
    // Ground Vehicles Filter
    if (PlaneFilter.groundVehicles === 'filtered') {
        if (
            (typeof this.category === 'string' && this.category.startsWith('C')) ||
