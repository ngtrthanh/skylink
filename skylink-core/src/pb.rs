...
fn build_pb_inner(store: &Arc<Store>, bbox: Option<(f64, f64, f64, f64)>) -> Vec<u8> {
    ...
    let aircraft: Vec<readsb::AircraftMeta> = store.map.iter().filter_map(|entry| {
        ...
        Some(readsb::AircraftMeta {
            ...
            geom_rate: ac.geom_rate.unwrap_or(0),
            ias: ac.ias.unwrap_or(0) as u32,
            tas: ac.tas.unwrap_or(0) as u32,
            ...
            nic: ac.nic.unwrap_or(0) as u32,
            nac_p: ac.nac_p.unwrap_or(0) as u32,
            nac_v: ac.nac_v.unwrap_or(0) as u32,
            sil: ac.sil.unwrap_or(0) as u32,
            gva: ac.gva.unwrap_or(0) as u32,
            sda: ac.sda.unwrap_or(0) as u32,
